...
MUTANTS = [
    ...
    {'name': '97',
     'file': 'bench/report/select.py',
     'old': '            if held and all(any(_comparable(other, age, age_tolerance_hours) for other in held)\n                            for age in ages):',
     'new': '            if held:',
    ...
    {'name': '103',
     ...
     'old': '        return (tier_by.get(name, float("inf")), cpu, name)',
     'new': '        return (tier_by.get(name, float("inf")), cpu)',
     'test': 'tests.test_select.CoordinatorFindingsTests.test_name_breaks_tie_across_many_providers',
     'assert': 'self.assertEqual(canonical_order(records), sorted(names))'},
    {'name': '104',
     'file': 'bench/report/select.py',
     'old': '            if held and all(any(_comparable(other, age, age_tolerance_hours) for other in held)\n                            for age in ages):',
     'new': '            if held and any(_comparable(other, age, age_tolerance_hours) for other in held for age in ages):',
     'test': 'tests.test_select.CoordinatorFindingsTests.test_fresh_observation_is_not_absorbed_by_a_stale_one',
     'assert': 'self.assertTrue(keep, reason)'},
    {'name': '105',
     'file': 'bench/report/select.py',
     'old': '            decisions[provider] = (True, "; ".join(parts), axis)',
    ...
    },
    {
        "name": "118",
        "file": "bench/providers/docker/probe.py",
        "old": "    widget.feed(text)\n",
        "new": "    widget.feed(lowered)\n",
        "test": "tests.test_detect.DetectChallengeTests.test_widget_parser_preserves_attribute_value_case",
        "assert": "self.assertEqual(self.detect(200, {}, body), (",
    },
    {
        "name": "119",
        "file": "bench/providers/docker/probe.py",
        "old": '                    not render_values or "explicit" in render_values\n',
        "new": '                    not render_values or "explicit" in [value.lower() for value in render_values]\n',
        "test": "tests.test_detect.DetectChallengeTests.test_explicit_render_value_is_case_sensitive",
        "assert": "self.assertEqual(self.detect(200, {}, body), (",
    },
    {
        "name": "120",
        ...
        "new": '            if attributes.get("data-sitekey") or _CAPTCHA_WIDGET_CLASSES.intersection(classes):\n',
        "test": "tests.test_detect.DetectChallengeTests.test_empty_or_valueless_sitekey_is_interactive",
    ...
    },
    {
        ...
        "new": '                    if "render" in part\n',
        "test": "tests.test_detect.DetectChallengeTests.test_render_substring_is_not_a_render_parameter",
        "assert": "self.assertEqual(self.detect(200, {}, body), (",
    },
    {
        "name": "123",
        ...
        "new": '        return "&#" + digits\n',
        "test": "tests.test_detect.CharacterReferenceTests.test_title_reference_semicolon_separates_following_digit",
        "assert": 'self.assertEqual(self.probe._title("<title>&#65;6</title>"), "A6")',
    },
    {
        "name": "124",
        "file": "bench/providers/docker/probe.py",
        "old": "    return _DECIMAL_CHARREF.sub(replace, text)\n",
        "new": "    return _DECIMAL_CHARREF.sub(replace, text, count=1)\n",
