...
### Исправлено
- **Детектор челленджа больше не срабатывает на прозу.** «just a moment» ловится
  только в `<title>`; остальные маркеры тела дают `suspected` лишь при двух
  разных правилах; одиночный captcha-атрибут без заголовка, маркера тела или
  `403`/`429` остаётся `none` (имя правила всё равно возвращается).
  `cf-mitigated` по-прежнему решает в одиночку.
