...
- [x] **M16 — COMPLETED 18.09.2026 (release `ae72bfe` выкачен): ложная `captcha` на честном HTTP 200.**
  ...
  - M16a-fix6 `docs/specs/m16a-fix6-test-holes.md` — сделана, но В MAIN НЕ
    ВЛИТА до fix7 (один подслучай закрепляет неверное поведение, см. ниже):
  ...
  - M16a-fix7 `docs/specs/m16a-fix7-url-normalization.md` — СДЕЛАНА 18.09.2026
    (вершина `dd63f70`), В MAIN НЕ ВЛИТА: держится вместе с fix6 и fix8, потому
    что её собственная правка внесла два новых ложных `captcha` (см. fix8).
  ...
  - M16a-fix8 `docs/specs/m16a-fix8-browser-parity.md` — IN_PROGRESS 18.09.2026,
    ...
    СДЕЛАНА (вершина `42611a9`), В MAIN НЕ ВЛИТА: держится с fix6 и fix7 до
    fix9, потому что сама внесла регресс (см. ниже). Первая попытка встала по
  ...
  - M16a-fix9 `docs/specs/m16a-fix9-scheme-and-duplicates.md` — IN_PROGRESS
    ...
    руками. Критерии AC-190…AC-196, ворота 142.
