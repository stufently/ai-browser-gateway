...
- [ ] **M13 — IN_PROGRESS, директива владельца 17.09.2026** («доделывай всё до
  ...
  - M13c `docs/specs/m13c-deploy.md` (черновик, план прошёл Codex с 4
    исправлениями: откат при сбое `up`, сверка отката по manifest/ID образа,
    неперезаписываемые улики, строгий AC разбора аргументов) — выкладка после
    слияния; bizprofile без expected_text через deployed API.
