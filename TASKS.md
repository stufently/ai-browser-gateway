...
- [ ] **M11 — HTTP API и CLI.** Следующая разрешённая веха: token auth,
  ...
  **Заблокировано квотой Grok до выполнения probe.**
  ...
  Очередь сохранена до фактического reset подходящего backend. После reset
  одна обычная попытка, повторный quotaerror не циклировать. Не заменять
  противоположного автора проверок тем же cx, который будет писать продукт.
  В task-file подготовки обновить историческую квоту2% на актуальную перед
  новым запуском; контракт не менять. Финализировать execution-спеку из
...
- [ ] **M12 — сервис, пул и deployed-прогон.** Compose в
  ...
  **Текущие квоты:** Spark исчерпан до20.09.2026 15:59 (timezone неизвестен).
  ...
  Grok weekly=0 (первый запрос M11 15.09 около00:21UTC), 5h/reset неизвестны;
  прежние2% устарели. Обычный cx — окна неизвестны, ради статуса не вызывался.
  ...
  не указано; подтверждения reset после фактического отказа M11 нет.
  ...
  Без доступного противоположного backend обязательный независимый probe и
  мутации не выполнить. Новые модельные запуски остановлены до reset;
  активной реализации/приёмки нет. Проект ещё НЕ готов к использованию:
  M9/M10 приняты и опубликованы; API/CLI/сервис остаются в очереди.
  После сохранения/push управляющая сессия может закрыть только
  `cxc-ai-browser-gateway`; намеренное закрытие не восстанавливать автоматически.
