...
- [ ] **M10 — обязательное сравнение двух egress перед фиксацией лестницы.**
  Задание `docs/specs/m10-egress-measurement.md`; Grok в том же чистом клоне
  на f3c3913, новая попытка `t-505a5d41b116-a01`.
  Панель `gk-abg-m10-recon-20260914` пересоздана после завершения разведки;
  прежний watcher продолжает следить, лог `m10-recon-watch.log` в cache.
  ...
  production profiles не создаются. Пакет `m10-egress-result.md` в cache.
...
cf-fetch. Следующий шаг — принять ограниченный замер и заморозить спеку M10;
