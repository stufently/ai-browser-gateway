...
- [ ] **M12 — возобновлена по обновлённой директиве владельца15.09.**
  ...
  **Сейчас: M12a требует новой вехи после verify.** Пакет Grok сохранён;
  ...
  m12a-shutdown-probes, BASE=62de539, push DISABLED. После закрытия Grok
  запуск cx всё ещё заблокирован: оба cx-слота заняты чужими попытками.
  Очередь готова; при свободном общем/cx слоте запустить неизменную задачу.
  Чужие панели не трогать, лимиты/квоты не обходить.
  Фоновая очередь уже запущена: `m12/cx-capacity-watch.py`, PID
  `m12/cx-capacity.pid`, лог `m12/cx-capacity.log` вcacheниже. Раз в45s читает
  только metadata run_journal; при total<4 и cx<2 один штатный launch той же
  задачи с проверкой неизменного spec SHA и owner=cxc-ai-browser-gateway.
  Итог запуска `m12/cx-queued-launch.{json,log}`; при успехе сразу свой сторож
  `m12/watch-shutdown-probe.{log,pid}` вне каталога пакета. После одного вызова
  launcher очередь завершается; quota/отказ не циклирует. Пока running-попытки
  этого проекта отсутствуют, подготовленный клон/спека сохранены.
  ...
  **Текущие квоты:** Spark исчерпан до20.09.2026 15:59 (timezone неизвестен).
  ...
  подтверждение владельца15.09 выше теперь разрешает обычную попытку.
  ...
  приняты; дальнейший запуск возобновлён обновлённой директивой владельца.
