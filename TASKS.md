...
- [ ] **Инструменты из рилса `DdJ-MY6OITt` — что ещё взять в проект.** Рилс
  ...
  2. **MCP-сервер `scrapling[ai]`** (`mcp>=2.0.0`) — сравнить с нашим
     ...
     **IN_PROGRESS 19.09.2026 — веха M18** (`docs/specs/m18-mcp-stdio.md`):
     свой MCP-сервер на stdlib (решение владельца), stdio, инструмент
     `fetch_page` поверх `/v1/fetch`. Попытка M18 (55be93c) ОТКЛОНЕНА: 6/6
     AC, но живой Claude Code 2.1.278 не подключился (ответ версией
     `2026-07-28`, где нет `initialize`) и не видел текст страницы (только
     `structuredContent`). Исправление — M18-fix1
     (`docs/specs/m18-fix1-initialize-era.md`) ПРИНЯТ (8b6ff7f): живой Claude
     Code подключается и видит текст страницы. Ревью Codex нашло 4 расхождения
     с MCP 2025-11-25 (ping ждёт загрузку, ошибки аргументов не isError,
     `id:null`, апостроф в пароле) → M18-fix2
     (`docs/specs/m18-fix2-concurrency-errors.md`), панель
     `cx-abg-m18-fix2-20260919`. fix2 ПРИНЯТ (dd37a02): живой Claude Code —
     3 параллельных вызова, ошибки аргументов видны модели. Ревью Codex:
     регрессия — BrokenPipe из рабочего потока не завершает сервер
     (воспроизведено) → M18-fix3 (`docs/specs/m18-fix3-broken-pipe.md`), панель
     `cx-abg-m18-fix3-20260919`. В main M18 вольётся после fix3.
