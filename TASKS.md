...
- [x] **M23 — цели замера вне гита (поручение владельца 22.09.2026).**
  ...
  адресов, IP и ASN. Код сделал координатор по решению владельца: две
  попытки Grok погашены преждевременно — по логам `~/.grok/sessions` обе
  работали инструментами (около 25 и 33 вызовов), а я принял размер
  контекста в статус-строке за «цикл размышлений» (урок — в памяти проекта).
  Приёмка: 5 критериев, 11 новых тестов, 14 мутаций из 14 убиты. Ревью
  по решению владельца — координатор и Grok вместо Codex; обе вехи (M22,
  M23) исправлены по находкам, см. CHANGELOG. Замер 18 целей
  (6 публичных, 12 локальных) × 4 провайдера, 72 записи, опубликована только
  сводка по классам: `docs/research/09-local-targets.md` и пункт в README.
  Два соседних магазина с этого хоста не отвечали за 15 с — в цели не взяты.
- [x] **M22 — публичный README под поиск и цитирование (решение владельца
  22.09.2026).** Готово 22.09.2026, исполнитель Grok: у Codex кончился лимит
  аккаунта до 24.09. Четыре критерия перезапущены координатором — зелёные;
  ревьюеров в этот день не было ни одного (Codex без лимита, agy шесть раз
  ответил 503), приёмка держится на разборе диффа и двух машинных гейтах.
  Координатор поправил регрессию: первая редакция чекера запрещала в README
  любую строку `sha256:` и тем снесла пин публичного образа по дайджесту.
  Страница репозитория: тем 20 из 20 (добавлены `bot-detection`, `scraper`),
  homepage — страница пакета GHCR, карточка соцпревью 1280×640 отрисована и
  отдана владельцу (API для загрузки у GitHub нет).
...
- [x] **Поддомен под свой CF-стенд — выкачен (14.09.2026).** Токен CF не нужен:
  ...
  `targets.toml` (`valid=false` до настройки). ОСТАЁТСЯ, но УПИРАЕТСЯ ВО
  ВЛАДЕЛЬЦА (проверено 19.09.2026): (1) порт 443 на stand-host держит чужой сервис
  `ikra_thailand-caddy-1` — добавлять туда vhost нельзя, чужие compose-проекты
  не трогаем; обходной путь без чужого прокси — свой слушатель на порту, который
  CF проксирует сам (8443), тогда цель станет `https://abg-cf.example.net:8443/`;
  (2) WAF-правило Managed Challenge и Turnstile руками не сделать: токен CF
  лежит в CI octodns-репо, на хосте его нет, а без правила CF пускает как
  обычный сайт и стенд бессмысленен. Нужен либо токен с правами на WAF, либо
  правило, заведённое владельцем.
...
- [x] **Живой рилс для `login-instagram` — выдан (14.09.2026).** Владелец дал
  ...
  `valid=true`. Проверка инструментов на этом рилсе ЗАКРЫТА: цель
  `login-instagram` входит в матрицу шести целей каждой выкладки и берётся
  `patchright` (последний замер — выкладка M16c 18.09.2026, `ok=true`), то есть
  подпись и мета-разметка достаются, а само видео остаётся за стеной логина.
- [x] **Какие ещё боевые цели из рабочих проектов добавить.** Закрыто вехой
  M23 (22.09.2026): цели из рабочих проектов живут вне гита, публикуется только
  сводка по классам. До неё были взяты
...
- [x] **check-sites проверяет CF-сайты образом шлюза (поручение 19.09.2026).** Готово
  20.09.2026. Репозиторий шлюза открыт (история вычищена, бэкап старой — git bundle вне
  репо), образ `ghcr.io/stufently/ai-browser-gateway-oneshot` публичный (`0.1.0`, `latest`).
  В check-sites влита веха с fix1: `checker.py --challenged-out`, `cf_verify.py`, джоба
  `verify_challenged` с образом по дайджесту, `hc_ping_sites` ждёт и её. Открылась —
  сайт жив; не открылась — письмо «Url failed»; сбой инструмента (в том числе не
  запустившийся браузер) — красная джоба без письма.
  Предположение спеки («из пода Kubernetes результат может отличаться») снято живым
  прогоном 20.09.2026: пайплайны 276006 (push) и 276007 (расписание, `RUN_FLAGS=sites`)
  зелёные, под тянет образ по дайджесту, `bizprofile.net` открыт через `scrapling`
  за 21.7 и 23.7 с. Цикл: `check_sites` 66 с + `verify_challenged` 33–55 с (с подъёмом
  пода) — вдвое ниже десятиминутного окна; `hc_ping_sites` пингует после обеих джоб.
...
- [x] **M16 — COMPLETED 18.09.2026 (release `ae72bfe` выкачен): ложная `captcha` на честном HTTP 200.**
  ...
  - M16a-fix4 `docs/specs/m16a-fix4-format-charrefs.md` (находка Codex по fix3,
    ...
    machine_pass 7/7, 574 unit-теста и ворота шлюза 8/8 своим прогоном, Codex
    принял без находок, agy сломался второй раз за день (отчитался «запустил
    тесты в фоне» и вышел) — soft-fail. Живой стенд на настоящем HTTP API:
    до правки `markdown`/`links`/`meta` отдавали 500, после — 200 с осмысленным
    содержимым. Независимые мутации Grok: 14 kill / 1 survivor / 3 equivalent /
    0 invalid, откат убит; выживший — та же дыра `&#65;6`, что и в прогоне по
    детектору, уходит в fix5. ПРИНЯТА и перенесена на main 18.09.2026
    (cherry-pick `32ebb74`; слиянием нельзя — после распрямления истории BASE
    ветки на main отсутствует).
  - M16a-fix5 `docs/specs/m16a-fix5-test-holes.md` — только тесты, девять дыр
    (шесть из прогона по fix2, две по fix3, одна по fix4), плюс блок
    `if __name__ == "__main__"` перенесён в конец `tests/test_detect.py`:
    он стоял посреди файла, и прямой запуск выполнял 52 теста из 69 вместо всех.
    ПРИНЯТА и влита 18.09.2026 (merge `5a799ca`): machine_pass 6/6, 586 тестов,
    ворота 123 детектора и 9 шлюза своим прогоном, Codex и agy приняли без
    находок, мутации Grok 9 kill / 13 survivor / 1 equivalent (продуктовый код
    не менялся ни на байт).
  - M16a-fix6 `docs/specs/m16a-fix6-test-holes.md` — ВЛИТА в main 18.09.2026 в
    составе цепочки fix6→fix9 (merge `3ff4556`):
    machine_pass 5/5, 599 тестов, ворота 134 и 11 своим прогоном, agy принял,
    мутации Grok 13 kill / 0 survivor — все тринадцать тестов различают
    поведение, три спорные оси (`data-sitekey` на любом теге, iframe с
    `api.js`, boolean `class`) признаны корректными. Только тесты:
    тринадцать новых дыр из прогона по fix5,
    все воспроизведены координатором на main: порядок отсечения `#` и `?` в
    `src`; `;` как разделитель query; регистр ИМЕНИ параметра `render=`;
    пробелы вокруг его значения; `data-sitekey` на любом теге, не только
    `div`/`script`; разделение class-токенов по любому пробельному символу
    (таб/LF/CR); точное имя пути `recaptcha/api.js`, а не суффикс
    (`xrecaptcha/api.js` не виджет); `src` разбирается только у `script`, не у
    `iframe`; охрана `or ""` у boolean-атрибута `class` (без неё детектор
    падает на `<div class>`); `&65;` без `#` остаётся сырым; пустой `&#;`
    остаётся сырым — последние две оси в обоих файлах. Кода не касается.
  - M16a-fix7 `docs/specs/m16a-fix7-url-normalization.md` — ВЛИТА 18.09.2026
    (вершина `dd63f70`) цепочкой fix6→fix9; её собственная правка внесла два
    новых ложных `captcha`, закрытых в fix8.
    machine_pass 6/6 (AC-170…AC-175). BASE `6b2e007` (вершина fix6). Находка Codex по fix6,
    проверена координатором в живом Chrome через его же `new URL()`: браузер по
    стандарту URL удаляет табы/LF/CR ВЕЗДЕ и обрезает пробелы по краям, а мы
    разбираем `src` как есть. Поэтому `?render=explicit ` (пробел в конце),
    ` recaptcha/api.js?…` (пробел в начале), `\n` в конце и `\t` ВНУТРИ
    значения дают ложный `none` — страница с настоящим интерактивным виджетом
    принимается за контент. Пробел внутри значения (`render= explicit`)
    значим и остаётся невидимой v3 — это единственный верный из пяти входов.
    Требует ещё одной выкладки (M16c) после приёмки.
  - M16a-fix8 `docs/specs/m16a-fix8-browser-parity.md` — ПРИНЯТА и ВЛИТА
    18.09.2026, BASE `dd63f70` (вершина fix7), исполнитель cx. Четыре находки ревьюеров по
    fix7, все перепроверены координатором в живом Chrome, а не в `urllib.parse`:
    (1) буквальный NUL в значении атрибута браузер заменяет на U+FFFD, а мы его
    вырезали — `src="…render=explicit\x00"` давал ложный `captcha`;
    (2) `class` мы режем `.split()`, то есть по NBSP и вертикальной табуляции,
    а `classList` по ним НЕ режет — `class="g-recaptcha foo"` давал ложный
    `captcha`; обе — ровно тот дефект, ради которого затевалась M16, только
    рождённый её же правкой; (3) обратный слэш в `src` браузер приравнивает к
    прямому (`recaptcha\api.js` → `/recaptcha/api.js`), мы — нет, ложный `none`;
    (4) краевая обрезка регуляркой квадратична: 4000/8000/16000 внутренних
    пробелов — 0,092/0,382/1,235 с в Docker против 0,000015 с у `str.strip`,
    то есть недоверенная страница жжёт CPU. Критерии AC-180…AC-186, ворота 140.
    ВЛИТА 18.09.2026 (вершина `42611a9`) цепочкой fix6→fix9; сама внесла
    регресс, закрытый в fix9. Первая попытка встала по
    контракту и была права: AC-181 требовал `("access_denied", ("status_403",))`
    там, где `_CAPTCHA_ATTR` всегда добавляет `body_captcha` — недостижимое
    ожидание, дефект спеки координатора; спека исправлена (`340b1b2`), клон
    сброшен, прогон переделан начисто. Итог второй попытки: machine_pass 7/7 с
    перезапуском команд (ворота 140 за 193 с), мутации Grok 3 kill / 0 survivor /
    1 equivalent — equivalent доказан коммутативностью операций и 349 входами;
    тест на время убивает возврат регулярки (мутант 3,94–4,70 с при пороге 2 с),
    запас у рабочего кода ×1000 (1,55–1,93 мс), то есть не мигалка. Codex — НЕ
    ПРИНИМАТЬ (одна находка, подтверждена), agy сорвался четвёртый раз за сутки
    (вместо вердикта стена «waiting for tests») — soft-fail.
  - M16a-fix9 `docs/specs/m16a-fix9-scheme-and-duplicates.md` — ПРИНЯТА и
    ВЛИТА 18.09.2026, BASE `42611a9` (вершина fix8), исполнитель cx, решение
    владельца «fix9 на обе находки». Две находки, обе воспроизведены
    координатором в НАСТОЯЩЕМ Chromium (`abg-playwright:m2`), обе дают ложный
    `captcha` на HTTP 200: (1) регресс самой fix8 — замена `\` на `/` сделана
    безусловной, а браузер меняет слэш только для special-схем, поэтому
    безобидный `data:`-скрипт с комментарием `//…` стал `captcha` (на fix7 было
    `none`); находка Codex; (2) дублирующийся атрибут — токенизатор браузера
    берёт ПЕРВЫЙ одноимённый (`class="foo" class="g-recaptcha"` → `foo`), а
    `dict(attrs)` берёт последний; дефект существует с появления разбора
    атрибутов, найден координатором при чтении диффа. Все кортежи критериев
    посчитаны на прототипе правки (607 тестов зелёные), AC-191…AC-193 проверены
    руками. Критерии AC-190…AC-196, ворота 142. ПРИНЯТА и ВЛИТА 18.09.2026
    (merge `3ff4556`, вершина `6d454a0`): machine_pass 7/7 на перезапуске,
    613 тестов на main. Первый прогон приёмки дал `invalid` по AC-194 и AC-196 —
    это была МОЯ ошибка порядка: приёмка гонялась одновременно с ревью Codex в
    том же клоне, и мутанты наложились друг на друга; перезапуск в одиночку на
    чистом дереве дал 7/7. Codex — НЕ ПРИНИМАТЬ, находка проверена и оказалась
    старым дефектом (см. «Известное ограничение» в CHANGELOG), а не регрессом
    fix9. agy сорвался пятый раз за сутки, даже с прямым запретом ждать фоновые
    задачи в контексте — soft-fail. Мутации Grok: 2 kill / 0 survivor /
    0 equivalent; список special-схем перемерен в Chromium 151 независимо и
    совпал, сверх WHATWG браузер конвертирует ещё `chrome`, `chrome-untrusted`,
    `devtools` — со страницы недостижимо.
  - Известный край, НЕ чиним (решение владельца 18.09.2026): Chromium считает
    special-схемой ещё и `chrome:`, наш список — нет; со страницы такой скрипт
    не грузится, поэтому недостижимо.
  - M16b `docs/specs/m16b-deploy.md` — ВЫКАЧЕНА 18.09.2026, release `ae72bfe`.
    Координатор проверил независимо от панели: контейнер `api` healthy,
  ...
  - M16c `docs/specs/m16c-deploy.md` — ВЫКАЧЕНА и ПРИНЯТА 18.09.2026, release
    `db4fc35` (вершина main после слияния fix6→fix9), откат на `ae72bfe`.
    Везёт четыре случая браузерного паритета и правило первого дубля атрибута.
    Особенность: живой цели у этой правки НЕТ — lowendtalk починен ещё M16b,
    а подкладывать враждебную разметку в прод запрещено. Поэтому ценность
    проверяется офлайн на ВЫЛОЖЕННОМ файле детектора (AC-956: четыре фикстуры
    + бюджет времени), а живые проверки (AC-954, AC-955, AC-957) отвечают
    только на вопрос «не сломала ли выкладка то, что работало». Команду AC-956
    координатор прогнал руками на коде main — rc=0, и она же падает на коде
    fix7, то есть действительно различает поведение.
    Панель сдала 9/10 и остановилась по контракту на AC-954 (ротация egress):
    `worker_internal_error` после ms3. Разбор координатора: это ошибка САМОГО
    раннера (его воркер-контейнер, `tests/deployed_m12b.py:212`), под которой
    прячутся `worker_timeout`/`worker_connection`/`worker_failure`, а не отказ
    шлюза. Проверено лично: сервис на `db4fc35` (образ `abg-runtime:db4fc359171c`,
    оба контейнера running, api healthy); выложенный `probe.py` байт-в-байт
    равен коду релиза (`b87e4458…`); AC-956 прогнан координатором ПРЯМО НА
    ВЫЛОЖЕННОМ файле — rc=0; lowendtalk через боевой API `ok=true`/`curl_cffi`/
    `challenge=none`/`stop`; `compose.env.pre-m16c` на месте (0600, 462 байта).
    Профили при перепрогоне координатора: 14/15, сломан ms7 — а у панели
    ломались ms3 и ms14, то есть подводит не релиз, а конкретные прокси, и
    каждый раз разные. Перепрогон `--check-api-egress` координатором на том же
    релизе: `ok=true`, ротация direct → ms15 → ms1 → ms2 состоялась. Healthchecks
    `up` без падений. Выкладка принята; `accept_run` перезапустить нельзя —
    он коротко замыкается на `handoff_status: blocked` в отчёте панели, поэтому
    машинная часть здесь опирается на 9 pass панели плюс мои перепрогоны
    AC-954 и AC-956. ПРИНЯТА при НЕЗАКРЫТОМ AC-954: Codex на ревью закрытия
    показал, что мой вывод «это оснастка, а не шлюз» не доказан — в
    `worker_failure` попадает и разбор ответа шлюза, поэтому HTTP 500 дал бы
    тот же код; логи контейнеров пусты. Формулировки в README и отчёте
    исправлены на честные. Вторая находка Codex — мой же побочный ущерб:
    команда AC-956 грузила выложенный `probe.py` хостовым python без
    `PYTHONDONTWRITEBYTECODE`, и `__pycache__` осел ВНУТРИ каталога боевого
    релиза, из-за чего `verify_release` стал падать с `release_integrity_failed`.
    Каталог удалён, целостность обоих релизов (`db4fc35`, откат `ae72bfe`)
    проверена заново. Отдельной задачей: оснастка `tests/deployed_m12b.py`
    прячет настоящую причину отказа воркера под `worker_internal_error` —
    из-за этого честный сбой прокси неотличим от дефекта продукта.

- [x] **M17 — COMPLETED 19.09.2026 (влита в main, merge `b321b11`): оснастка сохраняет причину отказа воркера.**
  задача владельца «заводи веху на оснастку, чтобы причина отказа сохранялась».
  `docs/specs/m17-harness-failure-cause.md`, BASE `da97ec9`, исполнитель cx.
  Проверялка боевого сервиса схлопывает ЛЮБОЙ отказ своего воркер-контейнера в
  одну строку `worker_internal_error`, причём в общую ветку падает и разбор
  ответа шлюза — поэтому HTTP 500 шлюза неотличим от сбоя прокси. Это ровно то,
  на чём 18.09 встала выкладка M16c. Правка: воркер отдаёт `worker_check` с
  кодом `CheckError` и статусом (`api_http_error:500`), прочие падения — с
  именем класса исключения, а `worker_call` поднимает причину наружу через
  `redact()`. Секреты наружу не выпускаются: только статические коды и имя
  класса, без текста исключения. Продуктовый код не трогается вообще.
  Все кортежи и строки критериев посчитаны координатором на прототипе
  (613 тестов зелёные), AC-200 и AC-203 прогнаны руками до запуска панели.
  Итог: 6/6 критериев моим перепрогоном (620 тестов, 125 frozen probes, ворота
  шлюза 13/13), Codex — ПРИНЯТО без находок по прямому вопросу об утечке
  секретов, agy — ПРИНЯТО. Мутации гонял координатор, а НЕ Grok: у того
  кончился баланс (HTTP 402 Payment Required), из-за чего панель честно встала
  по контракту на незакрытом cross-review — блокер внешний, к работе отношения
  не имеет. Пять мутантов, все убиты своими тестами: снятие ветки CheckError,
  потеря HTTP-статуса в коде, возврат к безликому `worker_internal_error`,
  снятие `redact` (упал ровно тест на утечку) и выдача текста исключения вместо
  имени класса. Восстановление исходника подтверждено SHA256 после каждой
  мутации.
...
- [x] **Ложные «Url failed» по bizprofile.net в check-sites (24.09.2026).**
  Scrapling проходил челлендж, но детектор принимал `__cf_chl_tk` из
  закодированной ссылки рекламного iframe за челлендж. Исправлено и выложено
  везде: образ one-shot `0.1.1` (check-sites, скилл `abg-fetch` через ансибль),
  боевой сервис на релизе `2fa1b21e9e38`. В check-sites непройденная защита
  теперь сбой проверки (красная джоба), а не письмо о падении.
- [x] **Оснастка выкладки после чистки публичного репо.** `scripts/abg-provision`
  строит хосты прокси как `ms%d.example.net`: перегенерировать им
  `secrets/proxies.toml` нельзя — рабочий домен надо брать параметром или из
  окружения, не из кода. `tests/deployed_m12b.py` зашит на `/home/user` и
  `ABG_INSTANCE='stand-host'`, у живого сервиса другие путь и инстанс, поэтому
  выкладку 24.09.2026 проверял руками (health, `example.com`, bizprofile 2/2).
  **Закрыто 24.09.2026:** `abg-provision --domain`, `deployed_m12b.py
  --service-root/--instance`; на живом сервисе прошли `--check-deploy` и
  `--check-bizprofile`.

- [x] **Инструменты из рилса `DdJ-MY6OITt` — что ещё взять в проект.** Рилс
  ...
  2. **MCP-сервер `scrapling[ai]`** (`mcp>=2.0.0`) — сравнить с нашим
     ...
     **ЗАКРЫТО 19.09.2026 — M18 + fix1…fix3, влито в main (5c28d61).** Свой
     MCP-сервер на stdlib, `fetch_page` поверх `/v1/fetch`; проверен живым
     Claude Code. Путь: M18 отклонена живым стендом (версия без `initialize`,
     текст не виден модели) → fix1; ревью Codex → fix2 (ping, isError, id,
     апостроф) → fix3 (BrokenPipe из потока). Край с полным stderr записан в
     CHANGELOG по решению владельца. Ревизия `2026-07-28` — будущая веха,
     когда её заговорит реальный клиент.
  3. **Адаптивные селекторы (`auto_save` / `adaptive=True`)** — слой извлечения
     ...
     **ЗАКРЫТО 23.09.2026 — не брать.** Замер на реальных редизайнах из Wayback
     ([docs/research/10-adaptive-selectors.md](docs/research/10-adaptive-selectors.md)):
     из восьми сломавшихся полей, что остались на странице, отпечаток вернул три (все — переименование
     классов PyPI), на пороге по умолчанию пять раз молча отдал чужой элемент,
     на `percentage=60` ложных нет. В истории наших скраперов чинок из-за смены
     чужой вёрстки нет. Если понадобится — `percentage=60` плюс проверка формы
     значения.
  ...
  5. **Разбор 18.09.2026 по второму указателю владельца на тот же репозиторий.**
     ...
     общего HTML честнее. **ЗАКРЫТО 24.09.2026 владельцем — не делаем:** своя
     конверсия уже есть (режим `markdown` в `gateway/format.py`, у всех
     провайдеров), чужая нужна не была.
