...
- [ ] **M11 — HTTP API и CLI.** Следующая разрешённая веха: token auth,
  ...
  **В очереди: после reset Grok — независимые мутации M11 на FINAL_SHA,
  до старта M12.** Контракт/BASE и остальные требования приёмки неизменны.
  Дополнительная зависимость машинного gate: accept_run.py:300 требует
  пару agy+grok для executor.backend=codex. Grok Build wrapper использует
  тот же OAuth CLI; при известном weekly=0 не вызывать для quota retry.
