...
- [ ] **M23 — цели замера вне гита (поручение владельца 22.09.2026).**
  Решение владельца: новые цели живут в `bench/targets/local.toml` под
  `.gitignore`, наружу — только классы и агрегированные цифры. Код (веха
  M23, исполнитель Grok): `--targets` на несколько файлов, команда
  `aggregate` без id/адресов/IP, места под локальные файлы. Первая попытка
  Grok 22.09 ушла в вырожденный цикл размышлений без вызова инструментов и
  погашена; вторая идёт. Данные собирает координатор: из 114 адресов
  мониторинга за Cloudflare 12, из них 10 — JS/GIF одного CDN (в замер не
  берутся), 1 страница без челленджа, 1 с челленджем (уже в публичных
  целях); шесть магазинов владельца; пять официальных аккаунтов соцсетей.
  Два соседних магазина с этого хоста не отвечают за 15 с — в цели не взяты.
