...
`none` и среди сработавших правил нет `body_captcha`. Captcha-атрибуты
`src`/`class`/`id`/`name` сохраняют заголовок и прежний вердикт даже без
решающих CF-маркеров в теле. Остальные заголовки сохраняются, `None` остаётся
