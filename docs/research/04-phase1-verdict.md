...
В M17 оснастка сохраняет причину отказа в уликах как
`worker_internal_error:<internal_error>[:<cause>]`: `worker_timeout` и
`worker_connection` остаются без `cause`, `worker_check` передаёт статический
код проверки, а `worker_failure` — только имя класса исключения (например,
`ValueError`). Ошибки разбора ответа шлюза включают увиденный HTTP-статус:
`api_http_error:500`, `api_invalid_schema:200`, `api_invalid_json:200`;
остальные коды `parse_api` также дополняются статусом. Причина проходит через
`redact()`, текст произвольного исключения не передаётся. Это даёт улику для
будущих отказов; причину уже случившегося сбоя M16c восстановить нельзя.
