...
— ошибка САМОЙ оснастки, а не шлюза: `tests/deployed_m12b.py:212` так помечает
любой отказ своего воркер-контейнера, скрывая под ним `worker_timeout`,
`worker_connection` или `worker_failure`. Перепрогон тех же двух проверок
координатором на том же release:
...
Отваливаются РАЗНЫЕ профили от прогона к прогону, а при повторе проверка
проходит целиком — значит подводят конкретные прокси и оснастка, а не
выложенный код. Кроме того, ровно такой же одиночный отказ
...
падений. Выкладка принята; оснастку (сохранение настоящей причины ошибки
worker) чинить отдельной задачей.
