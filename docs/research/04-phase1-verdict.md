...
Детектор не бросает исключений разбора разметки наружу: преобразование
числовых сущностей в тексте отключено, ошибки `feed` перехватываются с
сохранением уже распознанного виджета, статуса и остальных меток. После
ошибки разбор остатка тела не продолжается; сущности в атрибутах могут
вызвать такую ошибку даже при `convert_charrefs=False`.
