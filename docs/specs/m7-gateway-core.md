...
| Ревью | запускает ПОСТАНОВЩИК (`ask-grok` + `ask-agy`) после того, как заберёт ветку. Машинерии `cross-review-v1` в этом репозитории НЕТ — не искать её и не запускать `review_run.sh` |
| Критериев | 6 |
