...
5. **Тесты `tests/test_detect.py`.**
   ...
   - Новые случаи: `class` с NBSP и с вертикальной табуляцией на 403 → вердикт
     `access_denied` БЕЗ `body_captcha_interactive`. Метка `body_captcha` при
     этом ОСТАЁТСЯ: `_CAPTCHA_ATTR` ищет подстроку `captcha` в самом значении
     `class`, это отдельное от виджета правило, и менять его эта веха не
     должна. Полное ожидание —
     `("access_denied", ("body_captcha", "status_403"))`. Дальше:
     `class` с табом/LF/FF/CR → виджет найден;
